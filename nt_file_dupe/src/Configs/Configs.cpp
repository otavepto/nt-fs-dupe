...
namespace ntfsdupe::cfgs {
    ...
    bool add_entry_file(Mode mode, const std::wstring& original, const std::wstring& target = std::wstring(), bool file_must_exist = false);
...
bool ntfsdupe::cfgs::add_entry_file(Mode mode, const std::wstring& original, const std::wstring& target, bool file_must_exist)
{
...
bool ntfsdupe::cfgs::add_entry_module(Mode mode, const std::wstring& original, const std::wstring& target)
{
    ...
    case ntfsdupe::cfgs::Mode::module_redirect: {
        ...
        if (_target_upper.size() != _original_upper.size()) {
            NTFSDUPE_DBG(L"  original and target size mismatch");
            return false;
        }
        if (_original_upper.compare(_target_upper) == 0) { // self redirection
...
bool ntfsdupe::cfgs::add_entry(Mode mode, const std::wstring &original, const std::wstring &target, bool file_must_exist)
{
    ...
    try {
        ...
        case ntfsdupe::cfgs::Mode::file_hide:
        case ntfsdupe::cfgs::Mode::file_redirect:
            return add_entry_file(mode, original, target, file_must_exist);
    ...
    try {
        ...
        for (const auto &item: j.items()) {
            try {
                ...
                if (add_entry(mode, original, target, file_must_exist)) ++added_entries;
            } catch (const std::exception &e) {
                NTFSDUPE_DBG(e.what());
            }
        }
